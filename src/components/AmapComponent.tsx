...
const AmapComponent: React.FC<AmapComponentProps> = ({ 
  ...
  useEffect(() => {
    ...
    attractions.forEach((attraction, index) => {
      ...
      if (!latitude || !longitude || isNaN(latitude) || isNaN(longitude)) {
        console.warn(`景点 ${attraction.name} 的坐标无效:`, { latitude, longitude });
        return; // 跳过无效坐标的景点
      }
  ...
  const drawRoute = (positions: any[]) => {
    ...
    // 构建途经点
    const waypoints = validPositions.slice(1, -1);
      ...
      driving.search(
        ...
        (status: string, result: any) => {
          if (status === 'complete') {
            ...
            drawFallbackRoute(validPositions);
          }
        }
      );
    } catch (error) {
